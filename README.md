...
3. **Set up the Sheet and Doc.** Create a Google Sheet with a `Dashboard` tab and these column headers: `Timestamp`, `Applicant Name`, `Applicant Email`, `Resume Link`, `GitHub URL`, `Portfolio URL`, `Decision`, `Weighted Score`, `Confidence`, `Strengths`, `Concerns`, `Hiring Notes`. Create a Google Doc for duplicate logging. Update the document IDs in the `Lookup Prior Evaluations`, `Append 'Evaluated' Row`, and `Log Duplicate to Doc` nodes.
